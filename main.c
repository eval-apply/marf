...
#define PulseStatus printf("Step#; %i \n Mode: %d \n Pulse1: %i \n \n", gSequenceStepNumber_1,gSequencerMode_1, (Steps[0][gSequenceStepNumber_1].b.OutputPulse1) );
...
uint16_t TrimmedMean(uint16_t readings[]) {
  uint16_t hi = 0;
  uint16_t lo = 4095;
  uint32_t acc = 0;
...
void ADC_IRQHandler()
{
	...
	if ( ADC_GetFlagStatus(ADC1, ADC_FLAG_EOC) == SET) {
		...
		if (  (ADC_POT_sel_cnt<=15)) {
					...
					//Printout slider 0 value to console
					//if ((ADC_POT_sel_cnt) == 0) {

						printf("RAW: : %i \n", (Steps[0][ADC_POT_sel_cnt].b.VLevel));
						//}
					...
					if ( (Steps[1][ADC_POT_sel_cnt].b.WaitVoltageSlider == 1) ) {
						...
						Steps[1][ADC_POT_sel_cnt].b.VLevel = (acc/NUMS);//&0xFE0;
					...
					if ( (unsigned int) (ADC1->DR) == (unsigned int) Steps[0][ADC_POT_sel_cnt].b.VLevel ) {
						...
						if(average_index[0][ADC_POT_sel_cnt] == NUMS) average_index[0][ADC_POT_sel_cnt] = 0;
										...
										Steps[0][ADC_POT_sel_cnt].b.VLevel = (acc/NUMS);//&0xFE0;

						//Steps[0][ADC_POT_sel_cnt].b.VLevel = TrimmedMean(average_array[0][ADC_POT_sel_cnt])&0XFe0;
...
void TIM4_IRQHandler()
{
	...
	StepWidth_1 = GetStepWidth(0, gSequenceStepNumber_1);
		...
		if ((gSequencerMode_1 == SEQUENCER_MODE_RUN)|| ((gSequencerMode_2 == SEQUENCER_MODE_ADVANCE)))
		{
			gStepWidth_1++;
		};
...
void UpdateModeSection(void)
{
  ...
  mLeds.b.Pulse1       	= ~mStep->b.OutputPulse1; //removed hack for Gate1 Gate2 leds
